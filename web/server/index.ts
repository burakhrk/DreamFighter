...
const attackRequestSchema = z.object({
  prompt: z.string().trim().min(1).max(500),
  character: z.object({
    name: z.string(),
    species: z.string(),
...
app.post('/api/generate/character', async (request, response) => {
  try {
    ensureClient()
    const { prompt } = characterRequestSchema.parse(request.body)
    ...
    response.json(applyCharacterRules(prompt, draft))
  } catch (error) {
...
app.post('/api/generate/attack', async (request, response) => {
  try {
    ensureClient()
    const payload = attackRequestSchema.parse(request.body)
    ...
    const draft = await generateStructuredOutput({
      ...
      userPrompt: [
        ...
        `Character species: ${payload.character.species}`,
        `Character theme: ${payload.character.theme}`,
        `Attack prompt: ${payload.prompt}`,
      ].join('\n'),
      validator: attackDraftSchema,
    })

    response.json(applyAttackRules(payload.prompt, payload.character, draft))
  } catch (error) {
    handleRouteError(error, response)
  }
})
...
function ensureClient() {
  if (!client) {
    throw new Error(
      'OPENAI_API_KEY is missing. Copy web/.env.example to web/.env and set your key.',
    )
  }
}
...
function applyAttackRules(
  prompt: string,
  character: z.infer<typeof attackRequestSchema>['character'],
